 ...
 *
 * @author Daina Daiana Jimenez Olivera
 * @author Héctor Daniel CErvantes Cañedo
 * Fecha : 14 Mar 2026
 */
...
public class SeguroVidaTemporal {
    ...
    // Método protegido para calcular probabilidad de muerte (qx)
    // Tabla de mortalidad simplificada con probabilidades ajustadas
    if (edadActual < 20) {
        return 0.0004;   // Probabilidad muy baja en infancia y juventud
    } else if (edadActual < 30) {
        return 0.0008;   // Aumenta ligeramente
    } else if (edadActual < 40) {
        return 0.0012;   // Incremento moderado
    } else if (edadActual < 50) {
        return 0.0025;   // Comienza a notarse el riesgo
    } else if (edadActual < 60) {
        return 0.0060;   // Aumento más significativo
    } else if (edadActual < 70) {
        return 0.0180;   // Riesgo considerable
    } else if (edadActual < 80) {
        return 0.0450;   // Riesgo alto
    } else if (edadActual < 90) {
        return 0.0950;   // Riesgo muy alto
    } else {
        return 0.2000;   // Para edades muy avanzadas (90+)
    }
}
